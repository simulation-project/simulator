...
handle_call({updateLocation, L}, _From, St) ->
    {IMSI,VLR,GT}=L,
    LU={6,1,IMSI,VLR,GT},
    {reply, updateLocation(LU), St};
...
%%%-----------------------------------------------------------------------------
%%---------------------------------------------------------------------------------
updateLocation({6,1,IMSI,VLR,CgPA}) ->
    insertVlr(IMSI, VLR, CgPA).
...
insertVlr(IMSI,VLR,CgPA)->
    ...
    {ok, Count} = pgsql:equery(C, "update hlr1 set vlr=$1 where imsi=$2", [VLR, IMSI]),
    io:format("updated ~p rows~n", [Count]),
    {ok, _C, ISD} = pgsql:equery(C, "select isd from hlr1 where imsi=$1",[IMSI]),
    ok = pgsql:close(C),
