...
%%--------------------------------------------------------------------------------
 sri(BNO,MSC1GT,HLRName)->
    ...
    {ok, _C, BIMSI} = pgsql:equery(C, Query, [BNO]),


    DB2 = "node_" ++ HLRName,
    Query2 = "select spc from " ++ DB2 ++ "  where gt=$1 ",
    {ok, _C, MSC1SPC} = pgsql:equery(C, Query2, [MSC1GT]),
    

  Query3 = "select vlr from " ++ DB ++ "  where isd=$1 ",
    {ok, _C, MSC2GT} = pgsql:equery(C, Query3, [BNO]),
   ...
   Query4 = "select spc from " ++ DB2 ++ "  where gt=$1 ",
    {ok, _C, MSC2SPC} = pgsql:equery(C, Query4, [NMSC2GT]),

    [{D}] =  MSC2SPC,
    NMSC2SPC = binary_to_list(D),
    ok = pgsql:close(C),
 
    MSRN = msc_app:receive_PRN({6,5,NBIMSI,NMSC2SPC}),
